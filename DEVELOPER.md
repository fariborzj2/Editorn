...
## ۷. تجربه کاربری و قابلیت‌های پیشرفته (Phase 5 - در حال توسعه)

این فاز شامل توسعه ویژگی‌هایی است که تجربه کاربری را به سطح استانداردهای جهانی نزدیک می‌کند:
- **`HistoryManager`**: کلاسی برای مدیریت تاریخچه وضعیت ادیتور (Undo/Redo) که در هسته (`EditorCore`) ثتب شده است.
- **`DragDropManager`**: کلاسی جهت پشتیبانی از گرفتن و جابجاکردن بلوک‌ها به صورت بصری.
- **بلوک‌های جدید**: شامل بلوک `Table` (جداول) و بلوک `Code` (قطعه کد). هر دو این کلاس‌ها اکنون در ساختار `blocks` اضافه شده و ثبت شده‌اند.
